...
def run_saldo_mestre(
    input_excel: str,
    output_excel: str = 'FILAS_DEFINITIVO_SALDO_MESTRE.xlsx'
):
    ...
    for col in ['PRODUCT PROPOSTO', 'NR_FILA']:
        if col not in filas.columns:
            filas[col] = ''
    filas[['PRODUCT PROPOSTO','NR_FILA']] = filas[['PRODUCT PROPOSTO','NR_FILA']].fillna('')

    # Mês da FILAS (Blued = primeira coluna)
    ...
    for _, neg in long_delta[long_delta['Delta'] < 0].iterrows():
        ...
        filas_idx = filas[(filas['PRODUCT']==cod_neg) &
                          (filas['PRODUCT SERIES']==modelo) &
                          (filas['MES']==mes) &
                          (filas['PRODUCT PROPOSTO']=='')].index.tolist()
        ...
        for idx in filas_idx:
            ...
            escolhido = candidatos_same[0] if candidatos_same else (candidatos[0] if candidatos else None)
    ...
    for prod, s in saldo.items():
        if s <= 0:
            continue
        modelo = delta.loc[delta['PRODUCT']==prod, 'PRODUCT SERIES'].iloc[0]
        for _ in range(s):
            n = next(inc)
    ...
    with pd.ExcelWriter(output_excel, engine='openpyxl') as writer:
        filas.drop(columns='MES', errors='ignore').to_excel(writer, sheet_name='FILAS', index=False)
        delta.to_excel(writer, sheet_name=delta_sheet, index=False)


if __name__ == '__main__':
    run_saldo_mestre('TABELA PADRAO - sugerir troca de código dO CICLO.xlsx')
